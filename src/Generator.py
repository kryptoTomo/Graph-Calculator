...
class Generator:
    ...
    def rand_digraph_edge_number(n, l):
        ...
        for i in range(l):
            edge=tuple(random.sample(range(n), 2))
            data['graph'][edge[0]].append(edge[1])
            data['edges_description'][(edge[0],edge[1])]={'weight': random.randint(0,99),'color': '#%02X%02X%02X' % (random.randint(0,255),random.randint (0,255),random.randint(0,255))}
        print(data)
        return AdjacencyList(data)
    @staticmethod
        ...
        for i in range(n):
            for j in range(n):
                if i!=j and random.random() <= p:
                    data['graph'][i].append(j)
                    data['edges_description'][(i,j)]={'weight': random.randint(0,99),'color': '#%02X%02X%02X' % (random.randint(0,255),random.randint (0,255),random.randint(0,255))}
        return AdjacencyList(data)
    @staticmethod
